...
const AppReview = ({ initialValues, handleFormSubmit, onCancel }) => {
    ...
    const sanitizeHtml = (html) => {
        return DOMPurify.sanitize(html, {
            ALLOWED_TAGS: ['a', 'b', 'i', 'u', 'strong', 'em', 'p', 'br', 'div', 'span', 'blockquote', 'h1', 'h2', 'h3'],
    ...
    const handleAddTask = (values) => {
        ...
        if (taskParticipants.length === 0) {
            ...
            if (cleanedValues.name && cleanedValues.instructions && questionList.length > 0) {
                const newTask = { ...cleanedValues, questions: questionList, assignedParticipants: taskParticipants };
    ...
    const handleParentInputChange = (html) => {
        ...
        setParentEditorState(sanitizedHtml);
    ...
    const handleChildInputChange = (index, value) => {
        ...
        newChildEditorStates[index] = sanitizedValue;
        setChildEditorStates(newChildEditorStates);
    
        const newChildQuestions = [...childQuestions];
        newChildQuestions[index] = sanitizedValue;
    ...
    const handleAddQuestion = () => {
        const sanitizedChildren = childQuestions.map((childQuestion) => sanitizeHtml(childQuestion)); // Sanitize child questions
        const sanitizedParentQuestion = sanitizeHtml(question.parentQuestion); // Sanitize parent question
    
        const children = sanitizedChildren.map((childQuestion, index) => ({
            index,
            question: childQuestion
        }));
    
        const newQuestion = { ...question, parentQuestion: sanitizedParentQuestion, children };
